...
function SearchAux(game, currentTurn) {
	...
	while (searchCalls < 50_000) {
		...
		console.log(`Depth (${depth++}), Score (${result})`, PrettyResult(bestIndex), searchCalls);
...
function PrettyResult(result) {
	result[1] = 	result[1] === 0 ? 'Q1' :
					result[1] === 1 ? 'Q2' :
					result[1] === 2 ? 'Q3' :
					'Q4';
	result[2] = result[2] === false ? 'Left' : 'Right';
	return result;
}
