...
function draw() {
	...
	// let result = Search(GamePieces, 2, PIECES.BLACK, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
	a = Date.now() - a;
...
function SearchAux(game, currentTurn) {
	...
	originalDepth = depth;
	let result = Search(game, depth++, currentTurn, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
	console.log('bestIndex', result);

	if (result === Number.MAX_SAFE_INTEGER) {
		console.log("Winning Move:", bestIndex);
	} else {
		while (searchCalls < 200_000) {
			searchCalls = 0;
			originalDepth = depth;
			result = Search(game, depth, currentTurn, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);

			iteriveDeepening = result[0];
			console.log(depth++, result, bestIndex);
		}
	}
...
function Search(game, depth, currentTurn, alpha, beta) {
	searchCalls++;
	if (depth <= 0) return Evaluate(game, currentTurn);
	...
	for (let i = 0; i < listOfMoves.length; ++i) {
		...
		let evaluationOfMove = -Search(GamePiecesWithMove, depth-1, nextTurn, -beta, -alpha);
...
function GetEmptyIndicies(game, targetColor) {
	...
	// 		let scoreA = CountColorsOnRowColDiag(game, a, targetColor);
	// 		let scoreB = CountColorsOnRowColDiag(game, b, targetColor);
